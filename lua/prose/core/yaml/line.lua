...
Line.new = function(raw_line, base_indent)
  ...
  self.indent = utils.count_indent(raw_line)
  ...
  self.raw_content = utils.lstrip_whitespace(raw_line, base_indent)
  self.content = utils.strip_whitespace(self.raw_content)
  return self
end
...
Line.is_empty = function(self)
  if utils.strip_comments(self.content) == "" then
    return true
  else
    return false
  end
