local Line = require("prose.core.yaml.line")
...
Parser.parse = function(self, str)
  ...
  while i <= #lines do
    ...
    elseif line.indent == current_indent then
      ...
      elseif utils.tbl_is_array(parent) and value_type == YamlType.ArrayItem then
        -- Add value to parent array.
        parent[#parent + 1] = value
      elseif utils.tbl_is_mapping(parent) and value_type == YamlType.Mapping then
        assert(parent ~= nil) -- for type checking
        -- Add value to parent mapping.
        for key, item in pairs(value) do
          -- Check for duplicate keys.
          if utils.tbl_contains_key(parent, key) then
...
Parser._parse_next = function(self, lines, i, text)
  ...
  if text == nil then
    ...
    text = utils.strip_comments(line.content)
  end

  local _, ok, value

  -- First just check for a string enclosed in quotes.
  if utils.has_enclosing_chars(text) then
...
Parser._try_parse_field = function(self, lines, i, text)
  local line = lines[i]
  text = text and text or utils.strip_comments(line.content)
  ...
  value = value and utils.strip_whitespace(value) or nil
  if value == "" then
    value = nil
  end
  ...
  if key ~= nil and value ~= nil then
    ...
    if type(value) == "string" and next_line ~= nil and next_line.indent > line.indent then
      ...
      while next_line ~= nil and next_line.indent == next_indent do
        local next_value_str = utils.strip_comments(next_line.content)
  ...
  if block_key ~= nil then
    ...
    while j <= #lines do
      ...
      if next_line ~= nil and next_line.indent >= item_indent then
        j = j + 1
        table.insert(block_lines, utils.lstrip_whitespace(next_line.raw_content, item_indent))
      else
        break
      end
    end
  ...
  while i <= #lines do
    ...
    if line.indent == item_indent then
      ...
      if value_type == YamlType.Mapping then
        for key, item in pairs(value) do
          -- Check for duplicate keys.
          if utils.tbl_contains_key(out, key) then
...
Parser._parse_inline_array = function(self, i, text)
  ...
  while string.len(str) > 0 do
    ...
    if vim.startswith(str, "[") then
      -- Nested inline array.
      item_str, str = utils.next_item(str, { "]" }, true)
    elseif vim.startswith(str, "{") then
      -- Nested inline mapping.
      item_str, str = utils.next_item(str, { "}" }, true)
    else
      -- Regular item.
      item_str, str = utils.next_item(str, { "," }, false)
    end
    ...
    str = utils.lstrip_whitespace(str)
  end
...
Parser._parse_inline_mapping = function(self, i, text)
  ...
  while string.len(str) > 0 do
    -- Parse the key.
    local key_str
    key_str, str = utils.next_item(str, { ":" }, false)
    if key_str == nil then
      return false, self:_error_msg("invalid inline mapping", i, text), nil
    end
    ...
    if vim.startswith(str, "[") then
      -- Nested inline array.
      value_str, str = utils.next_item(str, { "]" }, true)
    elseif vim.startswith(str, "{") then
      -- Nested inline mapping.
      value_str, str = utils.next_item(str, { "}" }, true)
    else
      -- Regular item.
      value_str, str = utils.next_item(str, { "," }, false)
    end
    if value_str == nil then
      return false, self:_error_msg("invalid inline mapping", i, text), nil
    end
    local value = self:_parse_inline_value(i, value_str)
    if not utils.tbl_contains_key(out, key) then
      out[key] = value
    else
    ...
    if vim.startswith(str, ",") then
      str = utils.lstrip_whitespace(string.sub(str, 2))
    end
  end
...
Parser._parse_string = function(self, i, text)
  if vim.startswith(text, [["]]) and vim.endswith(text, [["]]) then
    ...
    text = utils.string_replace(text, [[\"]], [["]])
  end
  return true, nil, utils.strip_enclosing_chars(utils.strip_whitespace(text))
end
...
Parser.parse_string = function(self, text)
  local _, _, str = self:_parse_string(1, utils.strip_comments(text))
  return str
end
...
Parser.parse_number = function(self, text)
  local ok, errmsg, res = self:_parse_number(1, utils.strip_whitespace(utils.strip_comments(text)))
  if not ok then
...
Parser.parse_boolean = function(self, text)
  local ok, errmsg, res = self:_parse_boolean(1, utils.strip_whitespace(utils.strip_comments(text)))
  if not ok then
...
Parser.parse_null = function(self, text)
  local ok, errmsg, res = self:_parse_null(1, utils.strip_whitespace(utils.strip_comments(text)))
  if not ok then
