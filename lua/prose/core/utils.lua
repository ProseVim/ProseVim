...
---Get the strategy for opening files
---
...
---@param anchor obsidian.file.HeaderAnchor
---@return string
util.format_anchor_label = function(anchor)
  return string.format(" ❯ %s", anchor.header)
end

---@param opts { path: string, label: string, id: string|integer|?, anchor: obsidian.file.HeaderAnchor|?, block: obsidian.file.Block|? }
---@return string
...
util.input = function(prompt, opts)
  ...
  local input = util.strip_whitespace(vim.fn.input({
    prompt = prompt,
